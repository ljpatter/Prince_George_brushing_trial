...
# Load data
dat_div_LA <- read.csv("Output/Tabular Data/mean_count_all_years_LA.csv")

# Create treatment column without remove original location column
dat1_div_LA <- dat_div_LA %>%
...
species_cols <- setdiff(names(dat2_div_LA), c("location", "year", "block", "treatment", "plot", "recording_date_time"))
...
# Save plot
ggsave(
  filename    = "Figures/Beta diversity/beta_div_LA.tiff",
  plot        = p_beta_stack_FINAL_LA,
  device      = "tiff",
  width       = 28,
  height      = 26,
  units       = "cm",
...
# ---- 5a) LONG table ----
beta_table_FINAL_LA <- beta_summary_FINAL_LA %>%
  mutate(
...
# Save
write.csv(beta_table_FINAL_LA,
          "Output/Tables/Beta_diversity_LA_Table_long.csv",
          row.names = FALSE)


# ---- 5b) INTERLEAVED table (metric header row then treatments) ----
beta_table_FINAL_LA_interleaved <- beta_table_FINAL_LA %>%
  group_by(Metric) %>%
...
# Save
write.csv(beta_table_FINAL_LA_interleaved,
          "Output/Tables/Beta_diversity_LA_Table_interleaved.csv",
          row.names = FALSE)
