...
dat_LA_100_wide_full <- visit_template %>%
  left_join(dat_LA_100_wide, by = c("location", "recording_date_time"))

# 3. Create spp cols 2
species_cols2 <- setdiff(names(dat_LA_100_wide_full), c("location", "recording_date_time"))

# 4. Replace NA with 0
dat_LA_100_wide_full <- dat_LA_100_wide_full %>%
  mutate(across(all_of(species_cols2), ~ tidyr::replace_na(., 0)))

# Create year column
dat_100 <- dat_LA_100_wide_full %>%
  mutate(
...
write.csv(dat_100,
          "Output/Tabular Data/test/count_all_years_LA_100.csv",
          row.names = FALSE)
...
# Load data
dat1_LA <- read.csv("Output/Tabular Data/test/count_all_years_LA_100.csv")
