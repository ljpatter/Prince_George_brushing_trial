# ---
# title: "Gamma diversity (LA): standardized Before/After + BACI contrasts vs NT"
...
## ============================================================
## 2) Bootstrap (LA): keep RAW replicates for BOTH cell + baci
## ============================================================

set.seed(123)
B <- 1000

loc_tbl   <- abundance_all_years_LA %>% distinct(location, treatment)
...
for (b in seq_len(B)) {
  ...
  boot_dat <- sampled_locs %>%
    left_join(abundance_all_years_LA,
              by = c("location", "treatment"),
              relationship = "many-to-many")
  
  res <- tryCatch(compute_baci_LA(boot_dat), error = function(e) NULL)
...
wide_cells_gamma_LA <- gamma_emm_table_LA %>%
...
gamma_baci_table_LA <- boot_baci_raw %>%
  filter(
...
gamma_report_table_LA <- wide_cells_gamma_LA %>%
  ...
  bind_rows(
    wide_cells_gamma_LA %>% distinct(Metric) %>%
...
# Print
gamma_report_table_LA

# Save
write.csv(gamma_report_table_LA, "Output/Tables/GD_LA_EMMplusBACI_report.csv", row.names = FALSE)
...
x_lim <- max(abs(c(plot_df_gamma_LA$LCL, plot_df_gamma_LA$UCL)), na.rm = TRUE)

p_baci_gamma_LA <- ggplot(
  plot_df_gamma_LA,
...
print(p_baci_gamma_LA)

# Save (cowplot saves transparent by default; force white via ggsave)
ggsave(
  filename    = "Figures/Gamma diversity/gamma_div_LA.tiff",
  plot        = p_baci_gamma_LA,
  device      = "tiff",
  width       = 28,
  height      = 26,
  units       = "cm",
